...
class Point {
    ...
    goToPoint(last : Point, paths, currentPoint : Point = null, pathID : string = uuidv4()) {
        ...
        if (paths[pathID] === undefined) {
            paths[pathID] = new TraversablePath();
        }

        // stop condition
        if (currentPoint === last) {
            paths[pathID].addPoint(currentPoint);
            return;
        }
        ...
        let nextAvailablePoints = currentPoint.connectedPoints.filter((p) => {
            ...
            if (p.isBig) {
                ...
                return paths[pathID].getVisit(p) <= p.smallPointMaxVisits-1 && p.id !== 'start';
            }
        });
        ...
        for (let i = 0; i < nextAvailablePoints.length; i++) {
            ...
            let actualPath = TraversablePath.newFromTraversablePath(paths[pathID]);
            nextPoint.goToPoint(last,paths, nextPoint, pathID);

            pathID = uuidv4();
            paths[pathID] = TraversablePath.newFromTraversablePath(actualPath);
...
let t1 = Date.now();
console.log('Solution 1: '+validPaths.length);
